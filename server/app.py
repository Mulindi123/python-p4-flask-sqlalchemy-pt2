...
def pet_by_id(id):
    ...
    if not pet:
        response_body = f'<h1> 404 error. Pet not found</h1>'
...
def owner_by_id(id):
    ...
    if not owner:
        response_body = f'<h1>404 error. Owner not found</h1>'
        response = make_response(response_body, 404)
        return response
    ...
    if not pets:
        response_body += f'<h1>Has no pets at this time</h1>'
    
    else:
        for pet in pets:
            response_body += f'<h1>Has pet {pet.species} named {pet.name}</h1>'
